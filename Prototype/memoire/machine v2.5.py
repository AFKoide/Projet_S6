...
## Ecrit dans la mémoire les valeurs stockées
    ...
    if ordre == "droite":
        ...
        vgauche = 0
        vdroite = 70
        while (int(moyenne(gauche) > seuils_capteurs["capteur_gauche_noir"]),int(moyenne(centre) > seuils_capteurs["capteur_centre_noir"]),int(moyenne(droite) > seuils_capteurs["capteur_droite_noir"])) == (0,1,0):
            buggy.motorOn("l","r",vgauche)
            buggy.motorOn("r","f",vdroite)
